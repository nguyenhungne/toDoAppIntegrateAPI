...
const fullNameErrorMessage = 'Tên của bạn không hợp lệ, vui lòng nhập kí tự không dấu!'

const fullNameValidCheck = validateSubmit(fullName, fullNameErrorMessage, regexFullName )
...
const userNameValidCheck = validateSubmit(userName, userNameErrorMessage, regexUserName ) 

 // 4. số điện thoại: 
 const regexPhoneNumber = new RegExp(/(84|0[3|5|7|8|9])+([0-9]{8})\b|(0|0[3|5|7|8|9])+([0-9]{8})\b/g);
 const phoneNumberErrorMessage = 'Số điện thoại của bạn không hợp lệ!'
 
 const phoneNumberValidCheck = validateSubmit(phoneNumber, phoneNumberErrorMessage, regexPhoneNumber )

 //5. password
 const regexPassword = new RegExp(/^[A-Za-z]\w{7,14}$/);
 const passwordErrorMessage = 'Mật khẩu của bạn không hợp lệ!'
 
 const passwordValidCheck = validateSubmit(password, passwordErrorMessage , regexPassword  )
    ...
    if (fullNameValidCheck && emailValidCheck && userNameValidCheck && phoneNumberValidCheck & passwordValidCheck ) {
